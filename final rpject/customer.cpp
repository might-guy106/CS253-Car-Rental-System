...
customer::customer(string name, int id, string pswd) {
	Name = name;
	ID = id;
	Password = pswd;
}
...
void customer::display_avail_cars(MYSQL* conn) {
...
void customer::display_my_rentings(MYSQL* conn) {
...
void customer::rent_request(MYSQL* conn) {

	stringstream ss;
	ss << ID;
	string temp_ID = ss.str();

	if (check_user_eligibility(conn, temp_ID,"Customers")) {
		string car_model;
...
void customer::return_request(MYSQL* conn) {
	...
	if (check_return_car(conn, car_model, temp_ID)) {
		...
		update_user(conn, car_model, temp_ID, new_cond, return_date,"Customers");
...
void customer::clear_due_request(MYSQL* conn) {

	stringstream ss;
	ss << ID;
	string temp_ID = ss.str();

	display_due(conn, temp_ID,"Customers");
	...
	if (choice == 1) {
		...
		string upd = "UPDATE Customers SET Fine_Due=0 WHERE ID=" + temp_ID + "";
		if (mysql_query(conn, upd.c_str())) {
			cout << "Error: " << mysql_error(conn) << endl;
		}
		else {
...
void customer::customer_functions(MYSQL* conn) {
